  ...
  return (
    ...
    >
       <div className="flex items-center gap-4 mb-3">
          ...
          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">{getLabel()}</span>
       </div>
       <p className="text-sm font-black text-slate-950 truncate uppercase tracking-tight">{item.content.message || item.content.milestone || item.content.trendName || item.content.insight || 'New Update'}</p>
       <div className="flex items-center justify-between mt-4">
          <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest">Node sync active</p>
    ...
    <div className="rounded-[4rem] bg-white border border-slate-100 p-10 shadow-premium premium-card overflow-hidden relative">
      ...
      <div className="flex items-center justify-between mb-10 relative z-10">
         <div className="flex items-center gap-4">
            ...
            <h3 className="text-2xl font-black text-slate-950 uppercase tracking-tighter">Live Pulse</h3>
         </div>
         <Link 
            to="/pulse" 
            className="text-[10px] font-black uppercase text-indigo-600 hover:text-indigo-700 transition-all tracking-[0.2em] underline decoration-indigo-200 decoration-4 underline-offset-4"
         >
           View Hub
         </Link>
      </div>
      ...
      <Link 
        to="/pulse"
        className="w-full mt-10 h-16 rounded-[1.5rem] bg-slate-950 text-white font-black text-[11px] uppercase tracking-[0.3em] flex items-center justify-center gap-4 hover:shadow-floating transition-all active:scale-95 group shadow-xl"
      >
