...
export const Sidebar = () => {
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-none py-2 px-3 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.label} 
              to={item.href}
              className={`flex items-center gap-4 px-3 py-3 rounded-lg relative transition-all group ${
            ...
            >
              ...
              <item.icon className="w-5 h-5 shrink-0 transition-transform group-hover:-translate-y-0.5" />
              <motion.span 
      ...
      <div className="p-4 border-t border-border/30 flex flex-col gap-3">
        <Link 
          to="/settings"
          className="flex items-center gap-4 px-3 py-2 text-muted-foreground hover:text-foreground transition-colors group"
          title={!isExpanded ? "Settings" : undefined}
        >
          <Settings className="w-5 h-5 shrink-0 transition-transform group-hover:rotate-45" />
          <span className={`font-medium text-sm whitespace-nowrap transition-opacity ${isExpanded ? "opacity-100" : "opacity-0"}`}>
            Settings
          </span>
        </Link>
        <div className="flex items-center gap-3 px-3">
          ...
          <div className={`flex flex-col whitespace-nowrap transition-opacity ${isExpanded ? "opacity-100" : "opacity-0"}`}>
        ...
        >
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            {isExpanded && <span className="text-sm">AI Chat</span>}
          </div>
        </button>
      </div>
    </motion.div>
